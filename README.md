...
- **Routing** — OpenAI deployments use Foundry's project-scoped `/openai/v1/responses` endpoint. It sends Responses items (including opaque encrypted reasoning items) and function tool results, allowing GPT reasoning to survive a tool loop under `azure-identity` auth. Anthropic deployments remain on `/anthropic/v1/messages`; other publishers remain on OpenAI-compatible Chat Completions. Newer GPT-5/o-series models use `max_completion_tokens` instead of `max_tokens` only on the legacy Chat Completions route; Responses uses `max_output_tokens`.
